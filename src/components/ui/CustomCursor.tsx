...
export default function CustomCursor() {
    ...
    useEffect(() => {
        ...
        const handleHoverStart = (e: MouseEvent) => {
            ...
            const isClickable = target.closest('button, a, input, [role="button"], .interactive');
            if (isClickable) {
                gsap.to(follower, { 
                    borderWidth: 2,
                    scale: 1.5,
    ...
    return (
        <>
            ...
            <div
                ref={followerRef}
                className="fixed top-0 left-0 w-10 h-10 border border-primary/40 rounded-full pointer-events-none z-[9998] -translate-x-1/2 -translate-y-1/2 hidden md:block transition-all duration-300"
