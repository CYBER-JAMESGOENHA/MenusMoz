        ...
        <div ref={rootRef} className="relative overflow-hidden selection:bg-primary/20">
            ...
            {!showOnlyFavorites && (
                <div className="max-w-7xl mx-auto px-4 mt-6 mb-2">
                    <h2 className="text-2xl md:text-3xl font-display font-black tracking-tighter text-text-main italic">Os Mais Pedidos</h2>
                    <p className="text-[10px] font-bold text-text-dim uppercase tracking-widest mt-1 border-l-2 border-primary pl-2 ml-1">Os clássicos que nunca falham</p>
                </div>
            )}
            
            {!showOnlyFavorites && (
                <section className="max-w-7xl mx-auto px-4 pb-2 relative group/dishcarousel">
                    <button 
                    ...
                    <div ref={dishGridRef} className="flex overflow-x-auto gap-4 lg:gap-5 pb-4 pt-1 no-scrollbar snap-x snap-mandatory">
                        {filteredRestaurants.map((rest, idx) => (
                            <div key={`dish-${rest.id}-${idx}`} className="dish-card-anim opacity-0 translate-y-8 shrink-0 w-[80vw] sm:w-[280px] lg:w-[250px] xl:w-[270px] snap-start">
                                <RestaurantCard
                                    restaurant={rest}
                    ...
                    <div ref={gridRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {filteredRestaurants.length === 0 
                            ...
                            : filteredRestaurants.map(rest => (
                                <div key={rest.id} className="restaurant-card opacity-0 translate-y-8 h-full">
                                    <RestaurantCard
                                        restaurant={rest}
                                        isFavorite={favorites.includes(rest.id)}
                                        toggleFavorite={toggleFavorite}
